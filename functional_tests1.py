...
class newUserLogin(unittest.TestCase):
	...
	def test_can_start_notetaker_and_retireve_login_page(self):
		...
		inputbox = self.browser.find_elemnet_by_id('id_login_name')
		self.assertEqual(inputbox.get_attribute('name_placeholder), 'Enter login name')
		inputbox.send_keys('Jane Doe')
		inputbox.send_keys(keys.ENTER)
		#
		##
		###
		header_text2 = self.browser.find_element_by_tag_name('h2').text
		self.assertIn('log in', header_text2)
		inputbox2 = self.browser.find_elemnet_by_id('id_login_pwd')
		self.assertEqual(inputbox2.get_attribute('pwd_placeholder), 'Enter login name')
		inputbox2.send_keys('Jane Doe')
		...
		# Jane Doe is need of medical attention quite often. So she visits the clinic often. Everytime she visits, she is given an ipad that
